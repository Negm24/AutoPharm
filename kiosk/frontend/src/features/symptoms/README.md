...
- `i18n/{en,ar}.ts` — already merged into the `kiosk` namespace by `src/i18n/index.ts`.
  Keep `ar.ts` typed as `Localised<typeof symptomsEn>` so `npm run typecheck` catches drift.
- Add `pages/`, `components/`, `symptomsStore.ts` alongside them as needed.
...
- `@/features/session/sessionStore` — the kiosk session. Key assessments off `session.sessionId`,
